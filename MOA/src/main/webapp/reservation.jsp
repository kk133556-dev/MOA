...
<%
    ...
    LocalDate soonCutoff = today.plusDays(2);
...
<div class="d-flex">
    ...
    <% if (isApp) { %>
        ...
        <div style="padding:18px 16px 24px; background:#F7F6FB; min-height:100vh;">
            <div style="font-size:19px; font-weight:800; color:#1E1B2E; margin-bottom:16px;"><i class="bi bi-calendar-check"></i> 예약 관리</div>

            <div style="display:grid; grid-template-columns:repeat(3,1fr); gap:8px; margin-bottom:16px;">
                <div style="background:#fff; border-radius:14px; padding:13px 10px; text-align:center;">
                    <div style="font-size:10.5px; color:#8b87a3; margin-bottom:4px;">다가오는 예약</div>
                    <div style="font-size:16px; font-weight:800; color:#1E1B2E;"><%= upcoming.size() %></div>
                </div>
                <div style="background:#fff; border-radius:14px; padding:13px 10px; text-align:center;">
                    <div style="font-size:10.5px; color:#8b87a3; margin-bottom:4px;">오늘 예약</div>
                    <div style="font-size:16px; font-weight:800; color:<%= todayCount>0?"#DC2626":"#16A34A" %>;"><%= todayCount %></div>
                </div>
                <div style="background:#fff; border-radius:14px; padding:13px 10px; text-align:center;">
                    <div style="font-size:10.5px; color:#8b87a3; margin-bottom:4px;">임박(2일내)</div>
                    <div style="font-size:16px; font-weight:800; color:<%= soonCount>0?"#F59E0B":"#16A34A" %>;"><%= soonCount %></div>
                </div>
            </div>

            <% if ("1".equals(request.getParameter("done"))) { %>
                <div class="alert alert-success py-2" style="font-size:12.5px; border-radius:12px;"><i class="bi bi-check-circle"></i> 예약이 등록됐어요!</div>
            <% } %>

            <button type="button" data-bs-toggle="offcanvas" data-bs-target="#resAddSheet" style="width:100%; background:#8B5CF6; color:#fff; border:none; border-radius:14px; padding:14px; font-weight:700; font-size:14px; margin-bottom:18px;">
                <i class="bi bi-plus-circle"></i> 새 예약 등록
            </button>

            <div style="font-size:13px; font-weight:700; margin-bottom:10px; color:#1E1B2E;">다가오는 예약</div>
            <% if (upcoming.isEmpty()) { %>
                <div style="text-align:center; padding:30px 0; color:#8b87a3;">
                    <i class="bi bi-calendar-x" style="font-size:28px; opacity:0.3;"></i>
                    <p class="mt-2 mb-0" style="font-size:12.5px;">예정된 예약이 없어요.</p>
                </div>
            ...
            %>
                <div class="res-card <%= cardClass %>">
                    <div class="d-flex justify-content-between align-items-start">
                        <div>
                            <b style="font-size:13.5px;"><%= r.getReservationDate() %> <%= r.getReservationTime() %></b>
                            <% if (isToday) { %><span class="badge bg-danger ms-1" style="font-size:9px;">오늘</span><% } else if (isSoon) { %><span class="badge" style="background:#F59E0B; font-size:9px;">임박</span><% } %>
                            <div style="font-size:12.5px; margin-top:4px;"><i class="bi bi-person"></i> <%= r.getCustomerName() %> · <%= r.getPhone() %> · <%= r.getPartySize() %>명</div>
                            <% if (r.getMenuOrder() != null && !r.getMenuOrder().isEmpty()) { %><div style="font-size:12px; color:var(--text-muted); margin-top:2px;"><i class="bi bi-basket"></i> <%= r.getMenuOrder() %></div><% } %>
                            <% if (r.getPrepaymentAmount() > 0) { %><div style="font-size:12px; color:var(--text-muted);"><i class="bi bi-cash"></i> 선결제 <%= String.format("%,d", r.getPrepaymentAmount()) %>원</div><% } %>
                            <% if (r.getMemo() != null && !r.getMemo().isEmpty()) { %><div style="font-size:12px; color:var(--text-muted);"><i class="bi bi-sticky"></i> <%= r.getMemo() %></div><% } %>
                        </div>
                        <span class="badge <%= statusBadge %>" style="font-size:10px; flex-shrink:0;"><%= statusText %></span>
                    </div>
        ...
        <div class="offcanvas offcanvas-bottom" tabindex="-1" id="resAddSheet" style="border-radius:20px 20px 0 0; max-height:88vh;">
            <div class="offcanvas-header">
                <h6 class="offcanvas-title"><i class="bi bi-plus-circle"></i> 새 예약 등록</h6>
